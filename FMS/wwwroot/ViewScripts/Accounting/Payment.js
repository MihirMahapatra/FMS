$(function () {
                        ...
                        $.ajax({
                            ...
                            success: function (result) {
                                if (result.ResponseCode == 302) {
                                    ...
                                    html += '<td style="width:15px">';
                                    html += '<div class="form-group">';
                                    html += '<input type="text" class="form-control" id="txtDrAmount" for="DrBalance_' + uniqueId + '" name="DrBalance">';
                                    html += '</div>';
                                    html += '</td>';
                                    html += '</tr>';
                                    var newRow = PaymentTable.row.add($(html)).draw(false).node();
                                    ...
                                    if (item1.Payments && item1.Payments.length > 0) {
                                        $.each(item1.Payments, function (index, payment) {
                                            ...
                                            if (payment.Fk_SubLedgerId != null) {
                                                $.ajax({
                                                ...
                                                $('input[for="DrBalance_' + uniqueId + '"]').val(payment.Amount);
                                            }
                                        });
                                    }
                                }
    ...
    //---------------UpdatePayments-----------------------------------//
    $('#btnUpdate').on('click', UpdatePayments);
